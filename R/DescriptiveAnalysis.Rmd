...
We present a descriptive analysis based on empirical marginal survival curves with the Kaplan-Meier estimator and evaluate the hazard curve for some covariates including the categories demographics, service information, and customer account information. As we can be seen, categories for each variable present particular behaviour and could be useful to understand the time to churn.  Although there is a large set of covariates available to explain the lapse risk, a group of them is not informative for the survival time under study.
...
### Gender
The lapse rate for the two categories of \texttt{Gender} in the data is nearly the same. In this case, we do not expected that Gender is significant to explain the lapse risk. 
...
See that no Senior citizen has lower propensity to churn when compared with senior citizen. Although this covariate apparently significant to explain the rate of lapsation over time, the SeniorCitizen covariate does not show significant in the modelling.
```{r}
preds <-   
...
The behaviour of customers that do not have internet service or DSL service present similar behaviour in terms of lapse. See that customer that have Fiber optic service has proposenty to churn earlier compared each others. As the hazard curve are quite similar for No and DSL categorie, we consider to aggregate in an unique category called "others". 
...
This covariate apparently does not enough to explain the time to churn. See that the instantaneous lapse risk are quite similar for Yes and No categories.
...
This covariate apparently does not enough to explain the time to churn. See that the instantaneous lapse risk are quite similar for Yes and No contegories.
...
This covariate presents collinearity with variable **InternetService**. See the dependence in the cateogory ``No InternetService''.
